  ...
  async getCars (config) {
    ...
    allCarElems.each((idx, element) => {
      ...
      if (config.isInit) {
        ...
        if ($(element).hasClass('show-entry-end') && this.availableCars.delete(carName)) {
          this.notAvailableCars.set(carName, carData)
          this.soldOut.push({ ...carData, type: 'soldOut' })
        }
        if(!$(element).hasClass('show-entry-end') && this.notAvailableCars.delete(carName)) {
          this.availableCars.set(carName, carData)
          this.newCars.push({ ...carData, type: 'recovered' })
        }
        if (!$(element).hasClass('show-entry-end') && !this.availableCars.has(carName)) {
          this.availableCars.set(carName, carData)
          this.newCars.push({ ...carData, type: 'new' })
        }
        if (!$(element).hasClass('show-entry-end') && JSON.stringify(this.availableCars.get(carName)) != JSON.stringify(carData)) {
          this.availableCars.set(carName, carData)
