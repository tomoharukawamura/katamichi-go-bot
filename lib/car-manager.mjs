...
export class CarManager {
  constructor() {
    ...
    this.newCars = new Map()
    this.soldOut = new Map()
  }

  reset() {
    this.availableCars.clear()
    this.notAvailableCars.clear()
    this.newCars.clear()
    this.soldOut.clear()
    this.html = null
  }
  ...
  async getCars (config) {
    ...
    allCarElems.each((idx, element) => {
      ...
      const carData = { carName, startShop, returnShop, condition, date, startArea, returnArea }
      if (config.isInit) {
        ...
        if ($(element).hasClass('show-entry-end')
          ...
          this.notAvailableCars.set(carName, carData)
          this.soldOut.set(carName, { ...carData, type: 'soldOut' })
        } else if (!$(element).hasClass('show-entry-end') && !this.availableCars.has(carName)) {
          this.availableCars.set(carName, carData)
          this.newCars.set(carName, { ...carData, type: this.notAvailableCars.delete(carName) ? 'recovered' : 'new' })
        } else if (!$(element).hasClass('show-entry-end') && JSON.stringify(this.availableCars.get(carName)) != JSON.stringify(carData)) {
          this.availableCars.set(carName, carData)
          if (!this.newCars.delete(carName)) this.newCars.set(carName, { ...carData, type: 'updated' })
        }
      }
    })
  }
