...
const checkCars = (name) => {
  for (const rareCarName of Object.keys(rareCars)) {
    if (name.replace(' ', '').replace(/^(.+)\d{3,4}$/, '$1').includes(rareCarName)) {
      return {
  ...
  return {
    color: '#36a64f',
    photo: null,
    icon: Object.entries(specialIcons).find(([nameKey]) => name.replace(' ', '').replace(/^(.+)\d{3,4}$/, '$1').includes(nameKey))?.[1] || null
  }
}
