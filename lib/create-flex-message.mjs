...
export const createMessage = (car, type) => {
  ...
  const message = `${startAreaName}→${returnAreaName}`
  ...
  return {
    type: 'flex',
    altText: titleMsg.concat(abolishedCars.some(name => car.carName.includes(name)) ? '　🚫生産終了🚫' : ''),
    contents: {
      type: 'bubble',
      ...headerContent,
      body: {
        ...
        contents: [
          {
            type: 'text',
            text: titleMsg.concat(abolishedCars.some(name => car.carName.includes(name)) ? '\n🚫生産終了🚫' : ''),
            wrap: true,
          },
          {
            type: 'box',
